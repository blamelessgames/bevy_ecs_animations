...
#[derive(Clone, Copy)]
pub struct Seq<T, C1: Curve<T>, C2: Curve<T>> {
    first_curve: C1,
    second_curve: C2,
...
pub struct ScaledOutput<C: Curve<f32>> {
    low: f32,
    high: f32,
    curve: C,
}

impl<C: Curve<f32>> ScaledOutput<C> {
    pub const fn new(low: f32, high: f32, curve: C) -> Self {
        ScaledOutput { low, high, curve }
    }
}

impl<C: Curve<f32>> From<(f32, f32, C)> for ScaledOutput<C> {
    fn from((low, high, curve): (f32, f32, C)) -> Self {
        Self { low, high, curve }
    }
}

impl<C: Curve<f32>> Curve<f32> for ScaledOutput<C> {
    fn domain(&self) -> Interval {
        self.curve.domain()
    }
...
pub struct TimeScaled<T, C: Curve<T>> {
    low: f32,
    high: f32,
    curve: C,
    _marker: PhantomData<T>,
...
impl<T, C: Curve<T>> TimeScaled<T, C> {
    pub const fn new(low: f32, high: f32, curve: C) -> Self {
        TimeScaled {
            low,
            high,
...
impl<T, C: Curve<T>> From<(f32, f32, C)> for TimeScaled<T, C> {
    fn from((low, high, curve): (f32, f32, C)) -> Self {
...
impl<T, C: Curve<T>> Curve<T> for TimeScaled<T, C> {
    fn domain(&self) -> Interval {
